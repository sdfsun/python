...
def right_info():
    ...
    for i in range(1,11):   #某一页中某公司的食品抽查记录
        food_name=driver.find_element_by_xpath('//*[@id="tu_data"]/tr['+str(i)+']/td[1]').get_attribute('outerHTML')
        #print(food_name)
...
if __name__=='__main__':
    ...
    if int(page) >= 5:
        ...
        i = 3 + int(page)
    try:
        right_info()        #获取某一抽查的详细信息
        while True:     #翻页
            driver.find_element_by_xpath('//*[@id="pager2"]/div/div/ul/li['+str(i)+']/a').click()
            time.sleep(2)
            right_info()
    except:
        print('结束！')
